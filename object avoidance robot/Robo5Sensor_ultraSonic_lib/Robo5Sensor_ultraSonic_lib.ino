...
void loop()
{
 ...
 avgSpeed=avgSpeed/(Val-Dis); 
  ...
  else if (((FrontSensor.read(CM) <= Dis) && (n==0) && (TopSensor.read(CM)>Height))||((FrontSensor.read(CM) > Dis) && (n==0) && (TopSensor.read(CM)<=Height)))                         //There is object so bot will check for right and turn right or check left and turn left
  {
   if ((LeftSensor.read(CM) > Dis) && (RightSensor.read(CM) > Dis)  && (n==0))
   {Right(turningspeed);}
   else if ((LeftSensor.read(CM) <= Dis) && (RightSensor.read(CM) > Dis)  && (n==0))
    {Right(turningspeed);}

    else if  ((LeftSensor.read(CM) > Dis) && (RightSensor.read(CM) <= Dis) && (n==0))
   {Left(turningspeed);}
    
    if (LeftSensor.read(CM) <= Dis && RightSensor.read(CM) <= Dis)      // if both diretion i have object present bot will move backward for interval amount of time then take uturn and if and object present at back it will stop
